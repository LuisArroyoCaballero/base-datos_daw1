...
SELECT nombre, cuatrimestre, creditos
FROM asignatura
WHERE cuatrimestre= 
(
SELECT cuatrimestre 
...
WHERE tipo='alumno' 
AND
YEAR(fecha_nacimiento)>=1990 
AND 
YEAR(fecha_nacimiento)<=2000
ORDER BY YEAR(fecha_nacimiento) ASC;

SELECT DISTINCT(id_asignatura),
AVG(
SELECT id_alumno FROM alumno_se_matricula WHERE id_asignatura=id_asignatura
) AS Num_matriculados
...
SELECT id_departamento, count(id_profesor) AS num_prof
FROM profesor
...
SELECT id_asignatura, AVG(id_alumno) AS media_alumno
FROM alumno_se_matricula_asignatura
;

SELECT COUNT(id_profesor) FOR  id_departamento
FROM profesor;
...
SELECT COUNT(id_alumno), id_asignatura
FROM alumno_se_matricula_asignatura
